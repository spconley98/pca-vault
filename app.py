...
# --- 4. THE RECORDER (The New, Stable Version) ---
if not API_KEY:
    st.warning("Enter API Key in the sidebar to start.")
else:
    st.write("Click the mic to start/stop recording:")
    # This recorder is more reliable on mobile/web servers
    audio_data = mic_recorder(start_prompt="⏺️ Record Note", stop_prompt="⏹️ Stop & Analyze", key='recorder')

    if audio_data:
        st.audio(audio_data['bytes'])
        
        with st.spinner("AI is analyzing your call..."):
            ...
            prompt = "Analyze this recording and return ONLY a JSON object with: 'summary' (text), 'tasks' (list of strings), and 'events' (list of {title, time})."
            ...
            try:
                ...
                with col2:
                    st.markdown('<div class="report-card"><div class="card-title">📅 Events</div>', unsafe_allow_html=True)
                ...
                st.error("The AI had trouble formatting the summary. Here is the raw text:")
                st.write(response.text)
            
            os.remove(tmp_path)

st.markdown("<p style='text-align: center; font-size: 0.8em; color: #999; margin-top: 50px;'>Woodland PCA Intelligence</p>", unsafe_allow_html=True)